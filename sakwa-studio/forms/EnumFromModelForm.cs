...
namespace sakwa
{
    public partial class EnumFromModelForm : Form
    {
        ...
        protected void AddBranchToAvailable(IBaseNode node)
        {
            switch (node.NodeType)
            {
                case eNodeType.Branch:
                    lbxAvailable.Items.Add(new ListBoxItem(node.Name, 1));
                    break;

                case eNodeType.Expression:
                    lbxAvailable.Items.Add(new ListBoxItem(node.Name, 2));
                    break;
            }

            foreach (IBaseNode n in node.Nodes)
                AddBranchToAvailable(n);
        }
        ...
        {
            lbxSelected.Items.Add(new ListBoxItem(tbxElement.Text, 3));
