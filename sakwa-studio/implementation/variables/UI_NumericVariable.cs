...
namespace sakwa
{
    public class UI_NumericVariable : NumericVariableImpl, ICustomTypeDescriptor
    {
        public UI_NumericVariable() : base()
        {
            DefineProperties();
        }
        ...
        {
            if (baseNode != null && baseNode is NumericVariableImpl)
            {
                Maximum = Math.Max(Maximum, (baseNode as UI_NumericVariable).Maximum);
                Minimum = Math.Max(Minimum, (baseNode as UI_NumericVariable).Minimum);
